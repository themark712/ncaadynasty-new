...
//$aryb[] = "/edit/rosters.php?t=" . $_REQUEST["t"];
...
$aryb[2][0] = "/edit/rosters.php?tid=" . $userTeamId;
$aryb[2][1] = "Rosters";
...
if ($m != "") {
    ...
    if ($m == "a") {
        ...
        $arya["teamid"] = $_REQUEST["t"];
        //$roster->insert($arya);
        redirect("editroster/" . $_REQUEST["t"]);
    }
    if ($m == "u") {
        $id = $_REQUEST["pid"];
...
if ($teamid != 0 && $teamid != "") {
    ...
    if ($result) {
        foreach ($result as $row) {
            ...
            //$table .= "<td>" . $list . "</td>";
            $table .= "<td><input type='text' class='list' style='width:40px' name='number' value='" . $row["number"] . "' placeholder='#' /></td>";
...
<section class="pagetitle">
        ...
        <?php
        if ($admin=1 && $_SESSION["prem"]==1) {
        ?>
        <form name="newplayer" class='mb-3' action="roster.php?t=<?= $teamid ?>&p=0&m=a" method="post">
