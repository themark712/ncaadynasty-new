...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_REQUEST["m"] == "a") {
        ...
        if ($pid != 0) {
            $sql="INSERT INTO dynpstatsreceiving (schid, playerid, receptions, yards, tds, rac, drops, long) VALUES (:schid, :playerid, :receptions, :yards, :tds, :rac, :drops, :long)";
    ...
    if ($_REQUEST["m"] == "u") {
        $sql = "UPDATE dynpstatsreceiving SET ";
    ...
    if ($_REQUEST["m"] == "d") {
        $sql = "DELETE FROM dynpstatsreceiving WHERE id=" . $_REQUEST["stid"];
        //echo $sql;
        $result = query($sql);
