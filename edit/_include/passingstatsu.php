...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_REQUEST["m"] == "a") {
        ...
        echo $pid;
        if ($pid != 0) {
            $sql="INSERT INTO dynpstatspassing (schid, playerid, attempts, completions, yards, tds) VALUES (:schid, :playerid, :attempts, :completions, :yards, :tds)";
    ...
    if ($_REQUEST["m"] == "u") {
        $sql = "UPDATE dynpstatspassing SET ";
    ...
    if ($_REQUEST["m"] == "d") {
        $sql = "DELETE FROM dynpstatspassing WHERE id=" . $_REQUEST["stid"];
        //echo $sql;
        $result = query($sql);
